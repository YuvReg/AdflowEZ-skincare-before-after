...
/**
 * BeforeAfter (v4) — fixed-aspect drag-to-compare slider.
 * Matches the full-homepage reference footprint: a 390:448 frame on mobile,
 * 1280:670 on desktop, with the before/after images authored at those exact
 * aspects (object-cover is a no-op, so nothing is cut). The circular handle is
 * filled blue with a white hand + two chevrons, so it reads as "drag me both
 * ways." Expand opens the same comparison, just bigger.
 *
...
function Slider({
  ...
  return (
    ...
    >
      <style>{`
        @keyframes tolBaPulse{0%{transform:scale(.9);opacity:.8}70%{transform:scale(1.35);opacity:0}100%{opacity:0}}
        .tolBaHandle::before{content:'';position:absolute;inset:-7px;border-radius:9999px;border:2px solid rgba(255,255,255,.65);animation:tolBaPulse 1.8s ease-out infinite;pointer-events:none}
      ...
      {/* HANDLE — blue fill, white hand + chevrons, the only draggable element (>=44px touch target) */}
      <div
        role="slider"
        tabIndex={0}
        ...
        onKeyDown={onKey}
        className="tolBaHandle absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-[50px] h-[50px] rounded-full shadow-lg flex items-center justify-center cursor-grab active:cursor-grabbing touch-none focus:outline-none focus:ring-2 focus:ring-offset-2"
        style={{ left: `${pos}%`, background: '#3B7CF4', borderWidth: '3px', borderStyle: 'solid', borderColor: '#fff' }}
      >
        <span className="flex items-center gap-[3px]" style={{ stroke: '#fff', fill: 'none', strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round' }}>
      ...
      {/* Before/After labels strip lives ABOVE the frame in the page card; corner pills kept here for the standalone component */}
      <div className="absolute top-3 left-3 bg-black/60 text-white text-xs font-semibold px-2.5 py-1 rounded-md pointer-events-none">Before</div>
...
export default function BeforeAfter(props) {
  ...
  // Fixed aspect: 390:448 on mobile, 1280:670 on desktop (matches the reference frame).
  const frame = 'aspect-[390/448] sm:aspect-[1280/670]';
